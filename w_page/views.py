...
class person_view(View):
    ...
    def post(self,request):
        ...
        data = {'return JsonResponse(data)': 'Ok'}
...
class patient_view(View):
    ...
    def post(self,request):
        ...
        Patient.objects.create(patient_id=jd['patient_id'], person = Person.objects.get(person_id=jd['person_id']))
...
class doctor_account_view(View):
    ...
    def get(self, request, doctor_account_id=0):
        if (doctor_account_id > 0):
            persons=list(DoctorAcount.objects.filter(doctor_account_id=doctor_account_id).values())
            if len(persons) > 0:
                person= persons[0]
    ...
    def post(self,request):
        ...
        DoctorAcount.objects.create(doctor_account_id=jd['doctor_account_id'], 
                                    doctor = Doctor.objects.get(doctor_id=jd['doctor_account_id']),
    ...
    def put(self,request, doctor_account_id):
        ...
        persons = list(DoctorAcount.objects.filter(doctor_account_id=doctor_account_id).values())
        if len(persons) > 0:
            person = DoctorAcount.objects.get(doctor_account_id=doctor_account_id)
            person.doctor_account_id = jd['doctor_account_id']
            person.doctor = Doctor.objects.get(doctor_id=jd['doctor_account_id'])
    ...
    def delete(self,request, doctor_account_id):
        persons = list(DoctorAcount.objects.filter(doctor_account_id=doctor_account_id).values())
        if len(persons) > 0:
            DoctorAcount.objects.filter(doctor_account_id=doctor_account_id).delete()
            data = {'msg':'deleted'}
        else:
            data = {'msg': 'not deleted'}
        return JsonResponse(data)
...
class user_account_view(View):
    ...
    def get(self, request, user_account_id=0):
        if (user_account_id > 0):
            persons=list(UserAccount.objects.filter(user_account_id=user_account_id).values())
            if len(persons) > 0:
                person= persons[0]
    ...
    def post(self,request):
        ...
        UserAccount.objects.create(user_account_id=jd['user_account_id'], 
                                    patient = Patient.objects.get(patient_id=jd['user_account_id']),
    ...
    def put(self,request, user_account_id):
        ...
        if len(persons) > 0:
            ...
            person.patient = Patient.objects.get(patient_id=jd['user_account_id'])
    ...
    def delete(self,request, user_account_id):
        persons = list(UserAccount.objects.filter(user_account_id=user_account_id).values())
        if len(persons) > 0:
            UserAccount.objects.filter(user_account_id=user_account_id).delete()
            data = {'msg':'deleted'}
        else:
            data = {'msg': 'not deleted'}
        return JsonResponse(data)
...
class user_phone_numbers_view(View):
    ...
    def get(self, request, patient_id=0):
        if (patient_id > 0):
            persons=list(PhoneNumbers.objects.filter(patient=patient_id).values())
            if len(persons) > 0:
                person= persons[0]
                data = {'user phones': person}
            else:
                data = {'msg': 'user account not found'}
            return JsonResponse(data)
        else:
...
class diagnostic_view(View):
    ...
    def get(self, request, patient=0):
        if (patient > 0):
            persons=list(PatientDiagnostic.objects.filter(patient=patient).values())
            if len(persons) > 0:
                person= persons[0]
                data = {'diagnostic': person}
            else:
                data = {'msg': 'not found'}
            return JsonResponse(data)
        else:
    ...
    def put(self,request, diagnostic_id):
        jd = json.loads(request.body)
        persons = list(PatientDiagnostic.objects.filter(diagnostic_id=diagnostic_id).values())
        if len(persons) > 0:
    ...
    def delete(self,request, diagnostic_id):
        persons = list(PatientDiagnostic.objects.filter(diagnostic_id=diagnostic_id).values())
        if len(persons) > 0:
            PatientDiagnostic.objects.filter(diagnostic_id=diagnostic_id).delete()
            data = {'msg':'deleted'}
        else:
            data = {'msg': 'not deleted'}
        return JsonResponse(data)
...
class vital_view(View):
    ...
    def get(self, request, patient_id=0):
        if (patient_id > 0):
            vitals=list(VitalSigns.objects.filter(patient=patient_id).values())
            if len(vitals) > 0:
                vital= vitals[0]
                data = {'vitals': vital}
            else:
